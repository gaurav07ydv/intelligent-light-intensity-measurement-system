...
// ================= Pins =================
#define LDR_PIN  34     // ESP32 ADC pin
#define LED_PIN  25     // MOSFET gate pin

// ================= LED Control =================
bool ledState = false;

// Limit around 175 lux with hysteresis
const float LED_ON_THRESHOLD  = 1.0;
const float LED_OFF_THRESHOLD = 180.0;
...
void loop() {
  ...
  float s2Lux = bh1750.readLightLevel();   // S2 = BH1750

  sensors_event_t event;
  tsl.getEvent(&event);
  float s1Lux = event.light;               // S1 = TSL2561
  ...
  Serial.print(" | CalLux: "); Serial.print(smoothCalLux, 1);
  ...
  if (millis() - lastDisplayUpdate >= displayInterval) {
    ...
    display.print("CalLux:");
    display.print(smoothCalLux, 1);

    display.setCursor(0, 40);
    display.print("Limit:175");
